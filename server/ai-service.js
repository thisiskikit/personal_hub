...
export async function runStructuredAi({ mode, systemPrompt, userPrompt, fallbackFactory, modelOverride }) {
  const modelName = modelOverride || GEMINI_MODEL

  if (!GEMINI_API_KEY) {
    return {
      ok: true,
      modelName: modelName || 'fallback-no-key',
